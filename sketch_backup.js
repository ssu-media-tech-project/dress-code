...
function drawWardrobeModal() {
  ...
  for (let i = 0; i < categories.length; i++) {
    ...
    // 탭 배경
    if (isActive) {
      fill(100, 200, 255); // 선택된 탭 - 파란색
    } else if (isTabHovered) {
      fill(220, 240, 255); // 호버된 탭 - 연한 파란색
    } else {
      fill(240, 240, 240); // 일반 탭 - 회색
    }
  ...
  // 선택된 탭의 옷 표시 영역
  let contentY = tabY + tabHeight + 20;
  let contentHeight = modalHeight - (contentY - modalY) - 100;
...
function calculateScore() {
  ...
  if (selectedClothes.length === 3) {
    if (wrongCount === 0) {
      ...
      scoreMessage = "아쉽네요. 계절을 더 고려해보세요. (" + score + "점)";
    }
  } else {
    scoreMessage = "상의, 하의, 신발을 모두 선택해주세요.";
    return;
  }
...
function mousePressed() {
  ...
  if (gameState === "game") {
    ...
    if (!isGameCompleted) {
      ...
      if (
        ...
        if (!isWardrobeOpen) {
          ...
          selectedTab = "top"; // 탭 초기화
        } else {
          selectedTab = "top"; // 모달 열 때 첫 번째 탭으로 초기화
        }
        return;
      }
    }
    ...
    if (isWardrobeOpen) {
      ...
      // 옷 선택 클릭 (현재 선택된 탭의 옷만)
      let contentY = tabY + tabHeight + 20;
      let currentCategory = categories.find((cat) => cat.key === selectedTab);

      if (currentCategory) {
        let categoryClothes = availableClothes.filter(
          (cloth) => cloth.category === currentCategory.key
        );
        ...
        let itemsPerRow = Math.floor(
          (modalWidth - 60) / (itemWidth + itemSpacing)
        );
        let row = 0;
        let col = 0;
